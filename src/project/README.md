...
| `version.ts` | `SPEC_FORMAT_VERSION` constant. Stamped onto every emitted SPEC.md frontmatter and sidecar JSON. CHANGELOG signposts bumps; the `safer-spec-migrate` skill walks committed artifacts across them. |
| `folders.ts` | `discoverFolders` (recursive walk of `index.ts`-bearing folders), `discoverImmediateSubfolders` (for SPEC.md's `## Children` section), `buildChildren` (merged files + subfolders list). |
