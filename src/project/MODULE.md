...
**Residual contract:** "callers must treat as opaque; cross-version comparisons go through the safer-spec-migrate skill" — _comparison logic is migrate's responsibility; the skill walks MODULE.md + sidecar artifacts during format-version transitions._
...
- [`__tests__/config.spec.test.ts`](./__tests__/config.spec.test.ts) — Branch coverage for the \`safer-spec.config.json\` loader and per-folder threshold resolver in \`project/config.ts\`. The resolver tests exercise the three-layer fallback (override &gt; defaultThresholds &gt; 0); the loader tests exercise the four real-world outcomes a project boot hits (missing file, malformed JSON, unknown key at root, unknown threshold key, valid) plus the \`excludeRootPrefixes\` validation split (safe prefixes roundtrip, unsafe shapes fail with \`ConfigError\`).
...
| `project-context-excludes-configured-root-prefixes` | `Inclusion` | `loadProjectContext` | folders and sources under a configured \`excludeRootPrefixes\` entry are absent from the snapshot while sibling folders survive discovery | implemented |
...
| `spec-format-version-non-empty-constant` | `Constant Equality` | `SPEC\_FORMAT\_VERSION` | \`SPEC\_FORMAT\_VERSION === "0.1.0"\` — the literal version stamp every MODULE.md frontmatter and sidecar JSON carries; the migrate skill keys committed artifacts on bumps of this string | implemented |
...
| `spec-format-version-typechecks-as-string` | `Typechecking` | `SPEC\_FORMAT\_VERSION` | \`SPEC\_FORMAT\_VERSION\` is a non-empty string — the stable label every emitted MODULE.md frontmatter and sidecar JSON stamps | implemented |
