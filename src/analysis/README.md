...
| `properties.ts` | `extractProperties` — reads test files for `itSpec.todo`/`itSpec.prop` call sites, parses their JSDoc directives, returns the `PropertyRow` list the SPEC.md `## Properties` table renders. Type: `ItSpecIssue` for parse failures. |
