 ...
 * @spec.claim a missing MODULE.md on disk fails with `MissingSpecPropertyError`
 */
itSpec.prop(
  ...
  () =>
    Effect.runPromise(
      Effect.gen(function* () {
        ...
        const exit = yield* Effect.exit(checkDrift(fs, "/nonexistent/path/MODULE.md", "any content"));
 ...
 * @spec.type Roundtrip
 * @spec.exports checkDrift
 * @spec.claim a MODULE.md whose bytes equal the regenerated bytes (with the `generatedAtSha:` line normalized away) succeeds — the SHA line is volatile and excluded from the byte-equality check
 */
itSpec.prop(
