...
Anything that reads, writes, or escapes content for the codemod's on-disk artifacts: `<folder>/SPEC.md` (markdown + YAML frontmatter) and `<folder>/.safer-spec/<slug>.json` (canonical sidecar) + `<slug>.execution.json` (Vitest reporter sidecar).

| File | Job |
|---|---|
| `emit.ts` | `emitMarkdown(analysis, meta)` — produces the canonical SPEC.md text from a `FolderAnalysis` + `SpecMeta`. Also: `computeTypeCoverage`, `findMissingPropertyTypes`, the `ExportEntry`/`PropertyRow`/`FolderAnalysis`/`SpecMeta` types. |
...
| `link-resolver.ts` | Internal-link resolver (`relativeToFolder`, etc.) for the SPEC.md `## Children` section and inline references. |
