/**
 * @specPurpose Author-facing test helper. Terminal domain — `itSpec` is
 ...
 *   directives above it (`@specProperty`, `@specType`, `@specExports`,
 *   `@specClaim`). `generate` walks `*.spec.test.ts` files, parses these
...
export interface ItSpec {
  /**
   * @specAssume "first positional `id` arg matches the `@specProperty` JSDoc directive value above the call site"
   *   reason: cross-check enforced by `validate --implemented`; mismatch
   *           is exit code 11 (MISSING_SPEC_PROPERTY).
   * @specGuarantee "registers the property as a Vitest todo placeholder under `id`"
  ...
  /**
   * @specAssume "JSDoc directives above this call match `id`, `meta.type`, and `meta.exports` member names"
   *   reason: cross-check enforced by `validate --implemented`.
   * @specGuarantee "registers a fast-check property under `id` that runs `body` against samples drawn from `arb`"
   *   reason: side-effect contract; runtime registration with Vitest.
   * @specResidualContract "fast-check seed and numRuns are inherited from Vitest config; no override here"
